...
def test_reader_keyboard_issue_navigation_moves_between_comics(page, browser_server):
    ...
    page.goto(
        f"{browser_server['base_url']}/reader/{seed['active_comic_id']}"
        f"?context_type=reading_list&context_id={seed['reading_list_id']}",
        wait_until="networkidle",
    )

    page.locator(".reader-container").wait_for()
    page.wait_for_function(
        "expectedId => document.querySelector('.reader-container')?._x_dataStack?.[0]?.meta?.next_comic_id === expectedId",
        arg=seed["completed_comic_id"],
    )

    page.keyboard.press("]")
    page.wait_for_url(f"**/reader/{seed['completed_comic_id']}?*")
    page.locator("img.reader-page").wait_for()
    page.wait_for_function(
        "expectedId => document.querySelector('.reader-container')?._x_dataStack?.[0]?.meta?.prev_comic_id === expectedId",
        arg=seed["active_comic_id"],
    )

    page.locator(".nav-zone.center").click()
    toolbar = page.locator(".reader-toolbar")
    toolbar.filter(has_text=seed["completed_comic_title"]).first.wait_for()
