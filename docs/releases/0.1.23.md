...
- Fixed active browser sessions so protected page navigation no longer falls back to the login screen merely because the short-lived `access_token` cookie expired before client-side refresh could run, while preserving a two-hour true idle logout.
