...
- Added focused route-map regression coverage for newer FastAPI router wrapper behavior.
...
- Pinned Python and test dependency versions so local environments, Docker builds, and CI resolve the same package set.
...
- Fixed frontend route-map generation to stay compatible with newer FastAPI route wrapping and root-path handling.
...
.venv\Scripts\python.exe -m pytest tests/api/test_home.py tests/api/test_comics.py
.venv\Scripts\python.exe -m pytest tests/core/test_route_map.py
```
