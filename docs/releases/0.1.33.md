...
- Home rail headings, default Smart List rail markers, and Advanced Search saved-search controls now use subtle Lucide SVG icons instead of emoji or text glyph markers for more consistent cross-platform sizing.
...
- Verified Home and Advanced Search render Lucide SVG icons, no longer use the previous emoji/text glyph markers in those controls, route default Smart List rail markers through the Lucide fallback, and preserve Home customization plus saved-search browser flows: `3 passed`; `3 passed`; `2 passed` via `.\.venv\Scripts\python.exe -m pytest tests\api\test_pages.py::test_home_page_uses_lucide_rail_heading_icons tests\api\test_pages.py::test_home_smart_list_default_icon_uses_lucide_fallback tests\api\test_pages.py::test_advanced_search_page_uses_lucide_saved_search_icons -q`, `.\.venv\Scripts\python.exe -m pytest tests\browser\test_home_customization.py -q`, and `.\.venv\Scripts\python.exe -m pytest tests\browser\test_accessibility_keyboard_and_crud.py::test_saved_search_can_be_created_and_deleted tests\browser\test_saved_search_pull_list_and_session_flows.py::test_saved_search_can_be_reapplied_from_load_menu -q`.
